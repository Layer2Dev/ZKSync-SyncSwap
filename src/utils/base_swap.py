...
class BaseSwap(ABC, Account):
    ...
    async def swap(self) -> Optional[Union[bool, str]]:
        ...
        balance = await self.get_wallet_balance(self.from_token, from_token_address)
        amount = await self.create_amount(self.from_token, from_token_address, self.web3, self.amount)

        if balance == 0:
            self.logger.error(f'🅾️ | Your balance is 0 | {self.wallet_address}')
            return 'ZeroBalance'
        ...
        if amount > balance:
            self.logger.error(f'📉 | Not enough balance for wallet {self.wallet_address}')
            return
        ...
        tx = await self.create_swap_tx(self.from_token, self.to_token, contract, amount_out, from_token_address,
                                       to_token_address, self.wallet_address, amount, self.web3)
        if tx is None:
            return
        ...
        self.logger.success(
            f'Successfully swapped {"all" if self.swap_all_balance is True and self.from_token.lower() != "eth" and self.use_percentage is False else f"{int(self.swap_percentage * 100)}%" if self.use_percentage is True else self.amount} {self.from_token} tokens => {self.to_token} | TX: https://explorer.zksync.io/tx/{tx_hash}'
        )
